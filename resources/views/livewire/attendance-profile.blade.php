<div class='row'>
    <div class="col-md-4">
        @if ($curr_student)
            <img src="{{asset('/images/'.$curr_student->image)}}" class="img-fluid rounded-start" alt="..." width="300">
        @else
            <img src="data:image/jpeg;base64," class="img-fluid rounded-start" alt="..." width="300">
        @endif
    </div>
    <div class="col-md-8">
        @if ($curr_student)
            @php
                $stdTime = $curr_student->presences->last()->created_at;
                $tf = \Carbon\Carbon::parse($stdTime);
                $stf = \Carbon\Carbon::createFromFormat('H:i','07:10');
                $diff = $stf->diff($tf)->locale('id_ID');
                if($curr_student->presences->last()->attempts == 0){
                    $color = 'warning';
                    $statusMessage = 'Blocked';
                } else {
                    $status = $tf->lessThan($stf);
                    if($status){
                        $color = 'success';
                        $statusMessage = 'Tepat Waktu';
                    } else {
                        $color = 'danger';
                        $statusMessage = 'Terlambat ('.$diff.')';
                    }
                }
            @endphp
            <div class="card-body">
                <h5 class="card-title">Nama : {{$curr_student->name}}</h5>
                <p class="card-text">Kelas : {{$curr_student->grade->grade . ' ' . $curr_student->grade->name}}</p>
                <p class="card-text">Datang Waktu : {{ $tf->format('H:i A') }}</p>
                <p class="card-text mt-4">
                    <span class="badge text-bg-{{$color}}">
                        {{$statusMessage}}
                    </span>
                </p>
            </div>
        @else
            <div class="card-body">
                <h5 class="card-title">Nama : </h5>
                <p class="card-text">Kelas : </p>
                <p class="card-text">Datang Waktu : </p>
                <p class="card-text mt-4"><span class="badge text-bg-success"></span></p>
            </div>
        @endif
    </div>
</div>
